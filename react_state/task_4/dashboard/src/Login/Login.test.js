...
describe("<Login />", () => {
    ...
    it('verify that after changing the value of the two inputs, the button is enabled', () => {
        const wrapper = shallow(<Login />);
        wrapper.find({ id: 'email' }).simulate('change', { target: { name: 'email', value: 'leosoplapuco29@gmail.com' } });
        wrapper.find({ id: 'password' }).simulate('change', { target: { name: 'password', value: 'Leo3110leo@' } });
