	...
	return (
    	<div className='content-notifications'>
			...
			<div className={`Notifications ${displayDrawer ? 'show' : 'hide'}`}>
				...
				<ul>
					{listNotifications.map(notification => ( <NotificationItem key={notification.id} type={notification.type} value={notification.value || 'default'} html={notification.html}/> ))}
				</ul>
			</div>
    	</div>
  	);
